...
static Ref_t create_detector(Detector& description, xml_h e, SensitiveDetector sens) {
  ...
  std::cout << "  -- Dimension: inner R " << innerR << ", outer R " << outerR << ", inner half Z " << halfZ
            << std::endl;
  ...
  */
  ...
  double shift_phi_neg =
      fabs(fabs(tmp_slope1 * outerR * cos(dphi_sec / 2.) + tmp_intercept1) - outerR * sin(dphi_sec / 2.));
  ...
  PolyhedraRegular outerFrame(nSec_phi, tilt_rad, outerR * cos(dphi_sec / 2.) - Cframe_thick,
                              outerR * cos(dphi_sec / 2.), 2 * halfZ_out);
  ...
  double vertices_sector[16] = {
      halfZ,      inner_width_pos,  halfZ,      -inner_width_neg,
      -halfZ,     -inner_width_neg, -halfZ,     inner_width_pos,

      halfZ_out,  outer_width_pos,  halfZ_out,  -outer_width_neg,
      -halfZ_out, -outer_width_neg, -halfZ_out, outer_width_pos,
  };
  ...
  for (int iz = 0; iz < nModule_z; iz++) {
    ...
    std::cout << ", width (min) = "
              << fabs(outerR_sector * cos(dphi_sec / 2.) / tan(theta_min_module + tilt_min) -
                      innerR_sector / tan(theta_min_module));
    std::cout << ", width (max) = "
              << fabs(outerR_sector * cos(dphi_sec / 2.) / tan(theta_max_module + tilt_max) -
                      innerR_sector / tan(theta_max_module))
              << std::endl;
    ...
    double height_module = (theta_max_module < M_PI / 2.) ? crystal_thick * fabs(sin(tilt_max_local))
                                                          : crystal_thick * fabs(sin(tilt_min_local));
    if ((theta_max_module - M_PI / 2.) * (theta_min_module - M_PI / 2.) < 0)
      height_module = crystal_thick;
    ...
    std::cout << "  Theta range: " << theta_min_module << ", " << theta_max_module << ", tilt angle " << tilt_min
              << ", " << tilt_max << ", local tilt angle " << tilt_min_local << ", " << tilt_max_local << std::endl;
    ...
    double vertices_frame[16] = {
        ...
        innerR_sector / tan(theta_min_module) + height_module / tan(tilt_min_local),
        outer_width_module_pos,
        innerR_sector / tan(theta_min_module) + height_module / tan(tilt_min_local),
        -outer_width_module_neg,
        innerR_sector / tan(theta_max_module) + height_module / tan(tilt_max_local),
        -outer_width_module_neg,
        innerR_sector / tan(theta_max_module) + height_module / tan(tilt_max_local),
        outer_width_module_pos,
    };
    ...
    for (int i = 0; i < 8; i += 2)
      std::cout << "    [" << vertices_frame[i] << ", " << vertices_frame[i + 1] << "], " << std::endl;
    std::cout << "  Outer surface vertices: " << std::endl;
    for (int i = 8; i < 16; i += 2)
      std::cout << "    [" << vertices_frame[i] << ", " << vertices_frame[i + 1] << "], " << std::endl;

    double vertices_crystal[16] = {
        innerR_sector / tan(theta_min_module) - Cseg_thick,
        inner_width_pos - Cseg_thick,
        innerR_sector / tan(theta_min_module) - Cseg_thick,
        -inner_width_neg + Cseg_thick,
        innerR_sector / tan(theta_max_module) + Cseg_thick,
        -inner_width_neg + Cseg_thick,
        innerR_sector / tan(theta_max_module) + Cseg_thick,
        inner_width_pos - Cseg_thick,

        innerR_sector / tan(theta_min_module) + height_module / tan(tilt_min_local) - Cseg_thick,
        outer_width_module_pos - Cseg_thick,
        innerR_sector / tan(theta_min_module) + height_module / tan(tilt_min_local) - Cseg_thick,
        -outer_width_module_neg + Cseg_thick,
        innerR_sector / tan(theta_max_module) + height_module / tan(tilt_max_local) + Cseg_thick,
        -outer_width_module_neg + Cseg_thick,
        innerR_sector / tan(theta_max_module) + height_module / tan(tilt_max_local) + Cseg_thick,
        outer_width_module_pos - Cseg_thick,
    };
